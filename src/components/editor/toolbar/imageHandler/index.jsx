...
const ImageHandler = ({
  ...
  return (
    <Box h={145} w="100%" pos="relative">
      {files.length > 0 && (
        <ActionIcon style={{zIndex:30}} size="sm" color="red" pos="absolute" top={10} left={10} onClick={removeImage}>
        <IconTrash size={14} />
      </ActionIcon>
      )}
      <Dropzone
      ...
      ></Dropzone>
      {files.length < 1 &&
        <Flex direction="column" align="center" justify="center" pos="absolute" top={0} left={0} style={{zIndex:20,borderRadius:"16px",background:"rgba(0,0,0,0.3)"}} h={145} w="100%" onClick={() => openRef.current?.()}>
          <IconUpload />
