...
const orderFinalReceiptSlice = createSlice({
  name: "orderfinalreceipt",
  initialState: {
    orderfinalreceipt: [], // Empty array to avoid undefined
    totalfinalreceipt: 0,
    loadingfinalreceipt: false,
    errorfinalreceipt: null,
  },
  reducers: {
    clearOrderFinalReceipt: (state) => {
      state.orderfinalreceipt = [];
      state.totalfinalreceipt = 0;
      state.errorfinalreceipt = null;
    },
  },
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchFinalReceipt.fulfilled, (state, action) => {
        state.loadingfinalreceipt = false;
        state.orderfinalreceipt = action.payload || []; 
        state.totalfinalreceipt = action.payload.totalPriceApply || 0;
      })
      .addCase(fetchFinalReceipt.rejected, (state, action) => {
        state.loadingfinalreceipt = false;
...
export const { clearOrderFinalReceipt } = orderFinalReceiptSlice.actions;
export default orderFinalReceiptSlice.reducer;
