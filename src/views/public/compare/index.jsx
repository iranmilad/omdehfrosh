...
function Compare() {
  ...
  useEffect(() => {
    if (compareListData && compareListData.length > 0) {
      console.log('Compare page - Products data:', compareListData);
      compareListData.forEach((item, index) => {
        console.log(`Product ${index + 1}:`, {
          id: item.id,
          title: item.general?.title,
          images: item.general?.images,
          imagesType: typeof item.general?.images,
          isArray: Array.isArray(item.general?.images),
          imageLength: Array.isArray(item.general?.images) ? item.general.images.length : 'N/A',
          firstImage: Array.isArray(item.general?.images) ? item.general.images[0] : item.general?.images,
          validImageUrl: getValidImageUrl(item.general?.images)
        });
      });
    }
  }, [compareListData]);
            ...
            <Table striped withRowBorders={false} verticalSpacing="sm">
              ...
              <Table.Tbody>
                ...
                <Table.Tr>
                  ...
                  {compareListData.map((item, index) => (
                    <Table.Td key={index}>
                      <ProductImageCell item={item} getValidImageUrl={getValidImageUrl} />
                    </Table.Td>
                  ))}
                </Table.Tr>
