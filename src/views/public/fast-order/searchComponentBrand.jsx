...
const SearchComponentBrandFastOrder = ({ 
  ...
  const [filterSettingsModalOpened, setFilterSettingsModalOpened] =
    useState(false);
  ...
  const { tableDataFromSavedFilters, loadingTableDataFromSavedFilters } =
    useSelector(
      (state) =>
        state.fastOrderTableDataBrandModeSavedFilters ??
        EMPTY_SAVED_FILTERS_TABLE
    );
  ...
  const handleApiError = async (error, context = "") => {
    ...
    if (status === 401) {
      ...
      notifications.show({
        title: "خطای سرور",
        message:
          message || "خطای داخلی سرور رخ داده است. لطفا بعداً تلاش کنید.",
        color: "red",
        autoClose: 5000,
        position: "top-right",
      });
  ...
  const tableDataWhenChecked =
    reduxTableData &&
    typeof reduxTableData === "object" &&
  ...
  useEffect(() => {
    ...
    if (storedFilters) {
      try {
        ...
        const subcats =
          parsedFilters.filterBrandsCategorySubCategoryStorage || [];
  ...
  useEffect(() => {
    if (tableDataFromSavedFilters) {
      ...
      setFilterValues({
        ...DEFAULT_FILTER_VALUES,
        ...(tableDataFromSavedFilters?.filters || {}),
      });
  ...
  useEffect(() => {
    if (updateError) {
      if (
        ...
        updateError?.error &&
        typeof updateError.error === "object"
      ) {
        return;
      }
  ...
  useEffect(() => {
    ...
    if (
      isManualFilterUpdate.current ||
      sessionStorage.getItem("manualFilterUpdate") === "true"
    ) {
      return;
    }
    ...
    if (
      ...
      const matchingBrand = tableData.brands.find(
        (brand) =>
          ...
          (brand.slug &&
            (brand.slug === urlBrandName ||
              normalized(brand.slug) === normalized(urlBrandName)))
      );
      ...
      if (matchingBrand && matchingBrand.idBrand) {
        ...
        const hasSecondOrThirdRow =
          filterBrandsCategoryStorage?.length > 0 ||
          filterBrandsCategorySubCategoryStorage?.length > 0;
  ...
  useEffect(() => {
    ...
    if (searchType === "brand") {
      if (currentMode === "category") {
        ...
        const matchingBrand = tableData?.brands?.find(
          (brand) =>
            brand.idBrand && filterBrandStorage?.includes(brand.idBrand)
        );
        const slug = matchingBrand?.name ?? null;
        navigate(slug ? `/fastorder/brand/${slug}` : "/fastorder/brand", {
          replace: true,
        });
      } else if (
        currentMode === "brand" &&
        !urlBrandSlug &&
        filterBrandStorage?.length > 0 &&
        tableData?.brands?.length
      ) {
        const match = tableData.brands.find(
          (brand) => brand.idBrand && filterBrandStorage.includes(brand.idBrand)
        );
        if (match?.name)
          navigate(`/fastorder/brand/${match.name}`, { replace: true });
      }
    }
  }, [
    searchType,
    ...
    <>
      ...
      <Modal
        ...
        portalProps={
          ...
          document.getElementById("auth-modal-portal")
            ? { target: document.getElementById("auth-modal-portal") }
            : {}
        }
        styles={{
      ...
      >
        ...
        <Flex gap="sm" justify="flex-end">
          <Button onClick={handleLoginRedirect}>ورود به حساب کاربری</Button>
        </Flex>
      </Modal>
          ...
          >
            ...
            <Tabs.Tab
              ...
              style={{
                ...
                borderRadius: "6px",
                backgroundColor:
                  searchType === "category" ? "#093572" : "white",
          ...
          <Tabs.Panel value="brand">
            {!tableLoading && searchType === "brand" && tableData && (
              <SlideCategory
                ...
                isMobile={isMobile}
                isTablet={isTablet}
                key={`brand-${cookieUpdateTrigger}-${JSON.stringify(
                  filterBrandStorage
                )}-${JSON.stringify(
                  filterBrandsCategoryStorage
                )}-${JSON.stringify(filterBrandsCategorySubCategoryStorage)}`}
              />
            )}
          </Tabs.Panel>
        </Tabs>
